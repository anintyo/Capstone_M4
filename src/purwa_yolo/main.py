...
def calculate_safety_score(classcounts: dict) -> dict:
    """
    Calculate safety compliance score based on detected PPE.
    Score = (jumlah APD terdeteksi / total APD wajib) * 100
    """
    ...
    compliance_percentage = (len(detected_ppe) / len(required_ppe) * 100) if required_ppe else 0
    ...
    recommendations = generate_safety_recommendations(missing_ppe)
...
def generate_safety_recommendations(missing_ppe: set) -> list:
    """Generate actionable safety recommendations for missing PPE."""
    recommendations = []

    for ppe in missing_ppe:
        item = PPE_REQUIREMENTS[ppe]
    ...
    if not missing_ppe:
        recommendations.append({
            'priority': 'INFO',
            'message': "Pertahankan penggunaan APD yang sudah terdeteksi",
            'icon': '👍'
        })
...
def generate_pdf_report(safety_result: dict, image_name: str, annotated_image: np.ndarray = None) -> bytes:
    ...
    pdf.cell(box_width, box_height, '', align='C', fill=True)
    
    # Score text (without emoji)
    pdf.set_xy(x, pdf.get_y() + 5)
    pdf.cell(box_width, 10, f'{safety_result["score"]:.1f}%', align='C')
    ...
    pdf.cell(col_width, 10, f'APD Terdeteksi: {len(safety_result["detected_ppe"])}', border=1, align='C', fill=True)
    
    pdf.cell(10)  # Spacing
    
    # APD Kurang
    pdf.set_fill_color(255, 229, 229)  # Light red
    pdf.cell(col_width, 10, f'APD Kurang: {len(safety_result["missing_ppe"])}', border=1, align='C', fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    for ppe in safety_result['detected_ppe']:
        ...
        # Priority
        priority = ppe_info.get('priority', 'CRITICAL')
        pdf.set_text_color(220, 53, 69)
        pdf.cell(col_widths[2], 7, priority, border=1, align='C', fill=True)
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 8, 'Detail Kepatuhan', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    
    # APD Terdeteksi
    pdf.set_font('Helvetica', 'B', 10)
    pdf.set_text_color(0, 128, 0)
    pdf.cell(0, 6, '[OK] APD Terdeteksi:', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if safety_result['detected_ppe']:
        for ppe in safety_result['detected_ppe']:
            ...
            pdf.cell(0, 5, f'  - {ppe_info.get("name_id", ppe)}: {count} unit', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    else:
        pdf.cell(10)
        pdf.set_text_color(150, 150, 150)
        pdf.cell(0, 5, 'Tidak ada APD terdeteksi', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    pdf.set_font('Helvetica', 'B', 10)
    pdf.set_text_color(220, 53, 69)
    pdf.cell(0, 6, '[!] APD Tidak Terdeteksi:', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if safety_result['missing_ppe']:
        for ppe in safety_result['missing_ppe']:
            ...
            pdf.set_text_color(220, 53, 69)
            pdf.cell(10)
            pdf.cell(0, 5, f'  - {ppe_info.get("name_id", ppe)} [CRITICAL]', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    else:
        pdf.cell(10)
        pdf.set_text_color(0, 128, 0)
        pdf.cell(0, 5, 'Semua APD terdeteksi!', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if safety_result['recommendations']:
        ...
        pdf.set_text_color(0, 128, 0)
        pdf.cell(0, 5, '[OK] Tidak ada rekomendasi - Kepatuhan sempurna!', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Detect button
        if st.button("🔍 Detect Objects", type="primary", width='stretch'):
